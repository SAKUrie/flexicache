...
\lstset{
    ...
    numbers=none,#define CALL_MANAGED
...
int flexicache_load_block(void *func_addr, size_t size) {
    // 1. Check mapping table (cache hit)
    if (lookup_mapping_table(func_addr)) {
        stats.hit_count++;
        return 0;
    }
    ...
    // 5. Ensure instruction visibility (RISC-V fence.i)
...
This algorithm ensures that: (1) every function call triggers a mapping table lookup to detect residence (predictable hit/miss behavior), (2) space is reclaimed when needed via eviction, and (3) newly written instructions are made visible to the fetch unit.
...
\end{lstlisting}
\noindent \textbf{Note on Recursive Execution:} Since our current prototype uses a macro-based approach rather than full binary rewriting, internal recursive calls (e.g., \texttt{fibonacci} calling itself) are not intercepted by the \texttt{CALL\_MANAGED} macro. Consequently, the initial call jumps to I-Mem correctly, but subsequent recursive calls revert to executing from DRAM. Full execution within I-Mem for recursive functions is planned for Phase 2 (Preprocessor Development).
...
\item \textbf{Predictable Execution:} Every function call follows a deterministic path—table lookup, load on miss, execute. Cache behavior is explicitly managed, eliminating the variability of hardware replacement policies.
...
\item \textbf{WCET Foundation:} With explicit load operations, bounded table lookups ($O(n)$ where $n \leq 64$), and deterministic control flow, the system provides a solid foundation for future integration with static WCET analysis tools.
\end{itemize}
...
Developing Flexicache taught us several important lessons about low-level systems programming. First, bare-metal development requires meticulous attention to initialization order—setting the stack pointer before executing any C code is non-negotiable. Second, the distinction between physical memory addresses and logical memory regions is crucial when implementing software caching. Third, cache hit detection requires explicit state tracking (our mapping table), which hardware caches handle transparently but software must implement explicitly.

\subsection{Development Challenges}

We encountered several challenges during implementation. Initially, our test program crashed due to incorrect stack initialization, teaching us the importance of naked functions and inline assembly for bootstrap code. Debugging without an operating system forced us to develop alternative strategies, primarily UART-based printf debugging. We also learned that QEMU's RISC-V virt machine requires the \texttt{-bios none} flag for bare-metal programs, a detail not obvious from documentation.

\subsection{Team Collaboration}

This project reinforced the value of version control, containerization, and clear documentation. Our Docker-based environment ensured all team members could reproduce builds identically. The modular design—separating runtime library, linker script, and test code—allowed parallel development and simplified debugging. Regular code reviews helped catch subtle bugs, such as missing cache flushes after code loading.

\section{FUTURE WORK}

While Phase 1 successfully demonstrates functional software-managed caching, significant work remains to realize the full Flexicache vision:

\textbf{Phase 2: Preprocessor Development.} The next step is implementing the static analysis component. This involves parsing ELF binaries to extract control-flow graphs (CFGs), identifying function boundaries, and computing optimal code block sizes based on I-Mem capacity. The preprocessor will automatically insert load calls at function entries, eliminating the manual \texttt{CALL\_MANAGED} macros.

\textbf{Phase 3: Intelligent Eviction Policies.} Our current "flush all" eviction strategy is simple but inefficient. A production system should implement LRU (Least Recently Used) or working-set-based policies to minimize evictions. This requires tracking access timestamps in the mapping table and implementing a priority queue for eviction candidates.

\textbf{WCET Analysis Integration.} The ultimate goal is integrating Flexicache with WCET analysis tools. Since our system has bounded execution paths (table lookups are O(n) with n $\leq$ 64, memcpy is O(size)), WCET bounds could theoretically be computed statically. This would enable formal verification of real-time guarantees.

\textbf{Multi-Level Caching.} For larger programs, a two-level scheme (L1 SPM + L2 software cache in DRAM) could balance capacity and performance. The runtime library would manage both levels, paging between DRAM, L2, and L1 as needed.

\section{CONCLUSION}
\label{sec:conclusion}

This report presented the design and successful Phase 1 implementation of Flexicache, a software-managed instruction memory system tailored for real-time RISC-V embedded platforms. By replacing the conventional hardware cache with a software-controlled Scratchpad Memory (SPM) \cite{miller2006, gracioli2015}, Flexicache addresses the fundamental conflict between performance and predictability in safety-critical systems.

Our prototype, validated through a comprehensive five-phase test suite, demonstrated that a purely software-driven approach can achieve efficient instruction paging with improved analyzability and bounded timing behavior. The Runtime Library successfully managed the dynamic loading of code blocks with precise hit detection. Our design is structured to support WCET analysis because the runtime introduces bounded, explicit operations (table lookup, bounded-size copy), though formal WCET derivation is left as future work. While the current implementation relies on manual instrumentation, it proves that the overhead of software management is acceptable given the critical benefit of removing hardware cache replacement variability.

Moving forward, the integration of the static Preprocessor and automated binary rewriting will transform Flexicache from a proof-of-concept into a transparent toolchain. This project ultimately highlights that for domain-specific applications like medical sensors, the co-design of compiler and runtime systems offers a viable alternative to general-purpose hardware caching, paving the way for next-generation certifiable embedded systems.
